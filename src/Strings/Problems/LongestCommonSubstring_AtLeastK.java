...
public class LongestCommonSubstring_AtLeastK {
...
//                break;
//            }
//            String[] strs = new String[cant];
//            int k = cant / 2 + 1;
//            while (cant-- != 0) {
//                strs[cant] = in.nextLine();
//            }
...
//        }
        ...
        set = lcs(strs, k);
        System.out.printf("LCS(s) of %s with k = %d equals = %s\n", Arrays.toString(strs), k, set);
        // OUTPUT: LCS(s) of [AAGAAG  C, AGAAGT, CGAAGC] with k = 3 equals = [GAAG]
    ...
    /**
     * Finds the Longest Common Substring (LCS) between a group of strings. The
     * current implementation takes O(nlog(n)) bounded by the suffix array
     * construction.
     *
     * @param strs - The strings you wish to find the longest common
     * substring(s) between
     * @param K - The minimum number of strings to find the LCS between. K must
     * be at least 2.
     */
